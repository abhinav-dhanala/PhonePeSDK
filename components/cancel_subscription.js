...
module.exports = async function (req, res) {
    const { merchantUserId, subscriptionId } = req.body;

    // Log received payload
    console.log('Received payload in cancel_subscription:', req.body);

    // Validate input
    if (!merchantUserId || !subscriptionId) {
        ...
        let response = await axios.post(
            ...
            { request: Buffer.from(JSON.stringify(payload), 'utf8').toString('base64') },
            {
                headers: {
                    ...
                    'X-CALLBACK-URL': 'http://your-callback-url.com/callback' // Replace with your callback URL
                },
            }
        );
